...
public final class CRCUtil {
	...
	private final static int[] CRC_TABLE = new int[] { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
	...
	public static int crc16Cal(byte[] pData, int len) {
		if (null == pData) {
			return 0;
		}
		int index = 0;
		...
		int height8Bit;
		while (0 != len--) {
			height8Bit = (((crc & 0xFFFF) >> 8) & 0xFFFF);
			crc = ((bak & 0xFFFF) << 8);
			bak = crc;
			crc = ((bak & 0xFFFF) ^ (CRC_TABLE[((height8Bit & 0xFFFF) ^ (pData[index] & 0xFF)) & 0xFF] & 0xFFFF));
			bak = crc;
			index++;
		}
		return crc;
