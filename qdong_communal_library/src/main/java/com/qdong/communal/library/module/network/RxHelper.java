...
public class RxHelper {
  ...
  /**
    ...
    private Observable<?> autoLogin(LinkLinkApi service, HashMap<String, String> map) {
        ...
        try {
            Call<LinkLinkNetInfo> call = service.login(SharedPreferencesUtil.getInstance(context).getString(SharedPreferencesUtil.CLIENT_KEY,""),map);//同步调用,为何要同步?因为我只需要抓取cookie里面的sessionId
            response = call.execute();//执行
            ...
            if(response!=null&&response.body()!=null&&response.body().isSuccess()){
                ...
                AccessToken bean = JsonUtil.fromJson(response.body().getData(),AccessToken.class);
                SharedPreferencesUtil.getInstance(context).putString(SharedPreferencesUtil.ACCESS_TOKEN,bean.getAccessToken());

                RetrofitAPIManager.ACCESSTOKEN = bean.getAccessToken();//赋值
